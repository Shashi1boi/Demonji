...
$parsedUrl = parse_url($xtreamCredentials[0]['host']);
...
<body>
    <div class="container">
        ...
        <select id="serverSelect" class="server-select" onchange="fetchCategoriesAndChannels()">
            <?php foreach ($xtreamCredentials as $cred): ?>
                <option value="<?= htmlspecialchars($cred['id']) ?>"><?= htmlspecialchars($cred['name']) ?></option>
            <?php endforeach; ?>
        </select>
    ...
    <script>
        ...
        async function fetchCategoriesAndChannels() {
            ...
            const server = xtreamCredentials.find(cred => cred.id === serverId);

            if (!server || !server.host || !server.username || !server.password) {
                showPopup("Selected server details are missing or invalid.");
                return;
            }

            const baseURL = `${server.host}/player_api.php?username=${server.username}&password=${server.password}`;
            document.getElementById("loadingIndicator").style.display = "block";
            document.getElementById("categoryList").style.display = "none";

            try {                
                const categoryRes = await fetch(`feeder.php?url=${encodeURIComponent(baseURL + '&action=get_live_categories')}&server=${serverId}`);
                ...
                const channelRes = await fetch(`feeder.php?url=${encodeURIComponent(baseURL + '&action=get_live_streams')}&server=${serverId}`);
        ...
        function updatePlaylistUrl() {
            ...
            if (selected.length > 0) {
                playlistInput.value = `${basePlaylistUrl}?server=${serverId}&categories=${encodeURIComponent(selected.join(','))}`;
            } else {
                playlistInput.value = `${basePlaylistUrl}?server=${serverId}`;
            }
        }
        ...
        async function saveM3U() {
            ...
            const playlistUrlWithCategories = `${basePlaylistUrl}?server=${serverId}&categories=${encodeURIComponent(selected.join(','))}`;
